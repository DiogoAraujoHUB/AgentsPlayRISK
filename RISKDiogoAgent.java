...
public class RISKDiogoAgent extends Agent {
    ...
    private void attack_phase(ACLMessage decision) {
        ...
        while (true) {
            ...
            //Sends min amount of troops possible (90%) or a random amount (10%)
            int num_troops = 1;
            if(chance <= 90) { //Attack with minimum troops possible (90%)
                num_troops = 1;
    ...
    private void defense_phase(ACLMessage decision, String[] content) {
        ...
        if (board.get_country_troops(defending_country) >= 2) {
            ...
            //Max amount of possible troops to defend (90%)
            if(chance <= 90) {
                num_defenders = 2;
            } else { //Random amount (could still be 2) (10%)
                min = 1;
                max = 2;
    ...
    private void negotiation_phase(ACLMessage decision) {
        ...
        if (chance <= 25) { //Start negotiation (25%)
    ...
    private void form_phase(ACLMessage decision, String[] content) {
        ...
        //Check if alliance has been formed (65%)
        if (chance <= 65) {
            //Form alliance
