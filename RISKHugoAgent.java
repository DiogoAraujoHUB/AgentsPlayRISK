...
public class RISKHugoAgent extends Agent {
    ...
    private void attack_phase(ACLMessage decision) {
        ...
        while (true) {
            ...
            //Get chance for attack (possble attacks)
            min = 1;
            max = 100;
            int chance = r.nextInt(max) + min;

            //Sends max amount of troops possible (90%) or a random amount (10%)
            int num_troops = 1;
            if(chance <= 90) { //Attack with all troops possible (90%)
            	num_troops = troopsAvailable - 1;
            } else { //Attack with random amount of troops (10%)
            	min = 1;
            	max = troopsAvailable - 1;
            	num_troops = r.nextInt(max) + min;
            }
    ...
    private void defense_phase(ACLMessage decision, String[] content) {
        ...
        if (board.get_country_troops(defending_country) >= 2) {
            //Get chance for possible defenses
            int min = 1;
            int max = 100;
            int chance = r.nextInt(max) + min;

            //Least amount of possible troops to defend (90%)
            if(chance <= 90) {
                num_defenders = 1;
            } else { //Random amount (could still be 1) (10%)
                min = 1;
                max = 2;
    ...
    private void negotiation_phase(ACLMessage decision) {
        ...
        if (chance <= 0) { //Start negotiation (0%) (doesnt accept any negotiation)
    ...
    private void form_phase(ACLMessage decision, String[] content) {
        ...
        //Get chance that it will accept alliance forming (0%)
        int chance = r.nextInt(max) + min;

        //Check if alliance has been formed (0%) (alliances are refused)
        if (chance <= 0) {
            //Form alliance
