...
public class RISKJoaoAgent extends Agent {
    ...
    private void attack_phase(ACLMessage decision) {
        ...
        while (true) {
            ...
            min = 1;
            max = 100;
            int chance = r.nextInt(max - min) + min;

            //Sends max amount of troops possible (40%) or a random amount (60%)
            int num_troops = 1;
            if(chance <= 40) { //Attack with all troops possible (40%)
                num_troops = troopsAvailable - 1;
            } else { //Attack with random amount of troops (60%)
                min = 1;
    ...
    private void negotiation_phase(ACLMessage decision) {
        ...
        if (chance <= 10) { //Start negotiation (10%)
